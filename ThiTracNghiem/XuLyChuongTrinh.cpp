	...
	while (1)
	{
		...
		case 1:
		{
			...
			if (temp.compare("DONE") == 0)
			{
				...
				{
					THONGBAO(1, "SO CAU NHO HON TONG SO CAU");
					option = 0;
					break;
				}
				if (thoi_gian == 0)
...
bool ThemMonHoc(ListMH &dsmh)
{
	...
	while (1)
	{
		...
		case 0: // nhap ma mon
			...
			else if (strcmp(maMon.c_str(), "EXIT") == 0)
			{
				if (THONGBAO(3, "HUY THAO TAC?"))
					return 0;
			}
		...
		case 1: // nhap ten mon
			...
			if (is_Empty_CArray(tenMon.c_str()))
			{
				THONGBAO(1, "NHAP TEN MON HOC");
				//continue;
			}
			else if (strcmp(tenMon.c_str(), "EXIT") == 0)
			{
				if (THONGBAO(3, "HUY THAO TAC?"))
					return 0;
			}
		...
		if (temp == "LEFT" && key > 0)
			key--;
		else if (temp == "RIGHT" || temp == "DONE" && key < 5)
			key++;
		else if (temp == "EXIT")
		{
			if (THONGBAO(3, "HUY THAO TAC?"))
